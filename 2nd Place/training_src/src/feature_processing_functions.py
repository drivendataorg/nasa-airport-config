...
def ensure_datetime(
    df: pd.DataFrame, columns: Sequence[str] = DATETIME_COLS
) -> pd.DataFrame:
...
def feature_processing_function(func):
    ...
    def function_wrapper(
        date_range, *raw_feature_dfs, **kwargs
    ):  # ONLY PROVIDE A DATERANGE WITH REGULAR STEPS
        name = func.__name__
...
def process_true_rate(
    date_range, raw_operation_df: pd.DataFrame, maximum_lookback=30, op_type="dep"
) -> pd.DataFrame:
    ...
    all_minutes = pd.date_range(
        start=date_range["timestamp"].iloc[0],
        end=date_range["timestamp"].iloc[-1],
        freq="1T",
    ).to_frame(name="timestamp")
    minute_rates = all_minutes.merge(
        val_counts, how="left", left_index=True, right_index=True
    )
    minute_rates["counts"].fillna(0, inplace=True)
    frequency = int(
        (
            date_range["timestamp"].iloc[1] - date_range["timestamp"].iloc[0]
        ).total_seconds()
        / 60
    )
    ...
    for i in range(len(date_range)):
        ...
        if i * frequency >= maximum_lookback:
            ...
            output_row = [
                sum(counts_slice[bucket[0] : bucket[1]]) for bucket in buckets
            ]
...
@feature_processing_function
def process_projected_rate(
    date_range, raw_rate_df, rate_type="scheduled_runway_arrival", horizon=10
):

    start_date = date_range["timestamp"].iloc[0]
    end_date = date_range["timestamp"].iloc[-1]
    mask = (raw_rate_df["timestamp"] >= start_date) & (
        raw_rate_df["timestamp"] <= end_date
    )
    raw_rate_df = raw_rate_df[mask]
    raw_rate_df = raw_rate_df.drop_duplicates()
    raw_rate_df["t_delta"] = (
        raw_rate_df[f"{rate_type}_time"] - raw_rate_df["timestamp"]
    ).copy(deep=True).astype("timedelta64[s]") / 60
    raw_rate_df = raw_rate_df[raw_rate_df["t_delta"] > 0].copy(deep=True)
    raw_rate_df["minutes_of_hour"] = raw_rate_df["timestamp"].dt.minute
    raw_rate_df["mins_since_idx"] = (
        raw_rate_df["minutes_of_hour"] - (raw_rate_df["minutes_of_hour"] > 30) * 30
    )
    raw_rate_df["buckets_into_future"] = (
        (raw_rate_df["t_delta"] + raw_rate_df["mins_since_idx"]) / 30
    ).astype(int)
    raw_rate_df["since_start"] = (
        (raw_rate_df["timestamp"] - start_date).astype("timedelta64[s]") / (60 * 30)
    ).astype(int)
    raw_rate_df = raw_rate_df[raw_rate_df["buckets_into_future"] > 0]
    raw_rate_df = raw_rate_df[raw_rate_df["buckets_into_future"] < horizon]
    raw_rate_df = raw_rate_df.drop_duplicates(
        subset=["buckets_into_future", "since_start", "gufi"], keep="last"
    ...
    df_rate_matrix = pd.DataFrame(
        rate_matrix[:-1, :],
        columns=[f"next_{i*30 + 30}_min_{rate_type}" for i in range(horizon)],
    )
...
def process_time(date_range):
    ...
    # for d in range(7):
    #     df[f'day_of_the_week_{d}'] = (df['day_of_the_week']==d).astype(int)
...
def process_config_changes(date_range, raw_config_df, airport=""):
    ...
    config_df["previous_airport_config"] = (
        config_df["airport_config"].shift(1).fillna("UNK")
    )
    config_df["airport_config"] = config_df["airport_config"].fillna("UNK")
    config_df["change_in_this_period"] = (
        config_df["airport_config"] != config_df["previous_airport_config"]
    ).astype(int)
...
def process_weather(date_range, raw_weather_df):
    ...
    weather_df["east_wind"] = weather_df.wind_speed * np.cos(
        np.deg2rad(weather_df.wind_direction)
    )
    weather_df["north_wind"] = weather_df.wind_speed * np.sin(
        np.deg2rad(weather_df.wind_direction)
    )
    weather_df.drop(columns=["wind_direction", "wind_speed"])

    # Create a column to measure the relative prediction timedelta
    weather_df["prediction_delta"] = round(
        pd.to_timedelta(
            weather_df["forecast_timestamp"] - weather_df["timestamp"]
        ).dt.total_seconds()
        / 3600,
        1,
    )
    ...
    weather_df = weather_df.sort_values(
        ["timestamp", "prediction_delta"], ascending=(True, True)
    )
...
def process_runways(date_range, raw_config_df, airport=""):
    ...
    config_df[["departures_str", "arrivals_str"]] = config_df[
        "airport_config"
    ].str.split("_A_", expand=True)
    ...
    for dep_rway in departure_rways:
        config_df[f"{dep_rway}_departure"] = (
            config_df["departures_str"].str.contains(f"_{dep_rway}").astype(int)
        )
    ...
    for arr_rway in arrival_rways:
        config_df[f"{arr_rway}_arrival"] = (
            config_df["arrivals_str"].str.contains(f"_{arr_rway}").astype(int)
        )
    ...
    for rway in all_rways:
        ...
        if rway in departure_rways and rway in arrival_rways:
            config_df[col_name] = (
                config_df[f"{rway}_arrival"] | config_df[f"{rway}_departure"]
            )
...
def create_train_val_test_masks(complete_df, airport, train_labels, open_submission):
    train_labels_airport = train_labels[train_labels.airport == airport].copy(deep=True)
    open_submission_airport = open_submission[open_submission.airport == airport].copy(
        deep=True
    )

    train_timestamps = set(train_labels_airport.timestamp)
    val_timestamps = set(open_submission_airport.timestamp)

    start_date = datetime.datetime(2021, 10, 24, 16, 0, 0)
    end_date = datetime.datetime(2021, 10, 31, 16, 0, 0)
    prescreened_daterange = (
        pd.date_range(start=start_date, end=end_date, freq="60T")
    ).to_frame(name="timestamp")
    prescreened_timestamps = set(prescreened_daterange["timestamp"])

    complete_df["train_set"] = complete_df.index.isin(train_timestamps)
    complete_df["val_set"] = complete_df.index.isin(val_timestamps)
    complete_df["test_set"] = complete_df.index.isin(prescreened_timestamps)
    return complete_df
