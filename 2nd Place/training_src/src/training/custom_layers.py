...
import training.ak.ak_const
from training.ak.ak_utils import *
...
class MultiCategoryEncoding(preprocessing.PreprocessingLayer):
    ...
    def call(self, inputs):
        ...
        for input_node, encoding_layer in zip(split_inputs, self.encoding_layers):
            if encoding_layer is None:
                number = cast_to_float32(input_node)
                # Replace NaN with 0.
                imputed = tf.where(
                ...
                output_nodes.append(
                    cast_to_float32(encoding_layer(cast_to_string(input_node)))
                )
    ...
    def adapt(self, data):
        for index, encoding_layer in enumerate(self.encoding_layers):
            ...
            encoding_layer.adapt(data_column.map(cast_to_string))
...
class BertTokenizer(preprocessing.PreprocessingLayer):
    ...
    def __init__(self, max_sequence_length: int, **kwargs):
        ...
        self.tokenizer = FullTokenizer(
            vocab_file=constants.BERT_VOCAB_PATH, do_lower_case=True,
        )
...
class Transformer(layers.Layer):
    ...
    def call(self, inputs):
        if isinstance(inputs, (list, tuple)) and len(inputs) == 2:  # pragma: no cover
        ...
        attention_output = self._attention_dropout(attention_output)  # pragma: no cover
...
class DenseEinsum(layers.Layer):
    ...
    def call(self, inputs):
        ret = tf.einsum(self._einsum_string, inputs, self._kernel)  # pragma: no cover
...
class MaskedSoftmax(layers.Layer):
    ...
    def call(self, scores, mask=None):
        if mask is not None:  # pragma: no cover
            ...
            adder = (1.0 - tf.cast(mask, scores.dtype)) * -10000.0  # pragma: no cover
