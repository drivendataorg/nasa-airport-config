...
def LoadRawData(data_path, airports, start_time, end_time):
    ...
    for d in DATA_GROUPS:
        ...
        for airport in AIRPORTS:
            current_airport = pd.read_csv(os.path.join(data_path, airport, f'{airport}_{d}.csv'),
                                          parse_dates=["timestamp"])
            current_airport['airport'] = airport
            current_airport = current_airport[(current_airport['timestamp'] >= start_time) &
                                              (current_airport['timestamp'] <= end_time)]
            
            if d == 'tfm_estimated_runway_arrival_time':
            ...
            current_data = pd.concat([current_data, current_airport])
            
        raw_data[d] = current_data
    return raw_data
...
def CrossJoinDatesAirports(airports, start_time, end_time):
    ...
    timestampsDf = pd.DataFrame({'timestamp': pd.date_range(start_time, end_time, freq='15min')})
    timestampsDf['dummy_key'] = 0

    master_table = airportsDf.merge(timestampsDf, how='outer', on='dummy_key').drop(columns='dummy_key')

    return master_table


def ExtractAirportconfigFeatures(master_table, df):
    """
    ...
    :param pd.DataFrame df: Dataframe containing the airport configuration
    ...
    for airport in AIRPORTS:
        ...
        current = df[df['airport'] == airport].sort_values('timestamp').copy()

        # Add fictitious row in a future timestamp
        enlarge_configs = pd.DataFrame({'timestamp': [master_table['timestamp'].max()],
        ...
        current = current.set_index('timestamp').airport_config.resample('15min').ffill().reset_index()
        ...
        for r in RUNWAYS:
            current[f'feat_1_active_dep_{r}'] = current['feat_1_cat_airportconfig'].apply(
                lambda x: r in x.split('_A_')[0]).astype(int)
            current[f'feat_1_active_arrival_{r}'] = current['feat_1_cat_airportconfig'].apply(
                lambda x: r in x.split('_A_')[1]).astype(int)
            
            current[f'feat_1_active_dep_{r}R'] = current['feat_1_cat_airportconfig'].apply(
                lambda x: (r in x.split('_A_')[0]) and (r+'L' not in x.split('_A_')[0])).astype(int)
            current[f'feat_1_active_arrival_{r}R'] = current['feat_1_cat_airportconfig'].apply(
                lambda x: (r in x.split('_A_')[1]) and (r+'L' not in x.split('_A_')[1])).astype(int)
            
            current[f'feat_1_active_dep_{r}L'] = current['feat_1_cat_airportconfig'].apply(
                lambda x: (r in x.split('_A_')[0]) and (r+'R' not in x.split('_A_')[0])).astype(int)
            current[f'feat_1_active_arrival_{r}L'] = current['feat_1_cat_airportconfig'].apply(
                lambda x: (r in x.split('_A_')[1]) and (r+'R' not in x.split('_A_')[1])).astype(int)
        ...
        current.loc[(airport + ':' + current['feat_1_cat_airconfigother']).isin(CLASSES) == False,
                    'feat_1_cat_airconfigother'] = 'other'
        ...
        for i in [4, 8, 12, 16, 20, 24]:
            current[f'feat_1_nchanges_last_{i}'] = current[[f'feat_1_cat_previous_config_{j}'
                                                            for j in range(1, 25) if j < i]].nunique(axis=1)
...
def ExtractRunwayArrivalFeatures(master_table, df):
    """
    ...
    :param pd.Dataframe df: Dataframe containing the runway arrival data
    ...
    for airport in master_table['airport'].unique():
        current = df[df['airport'] == airport].copy()
        ...
        current = current.groupby('timestamp').agg({'indicator': 'sum',
        ...
        current.rename(columns={'feat_2_flight_ind1_last': 'feat_2_cat_flight_ind1_last',
...
def ExtractRunwayDepartureFeatures(master_table, df):
    """
    Extracts features based on past confirmed runway departure events
    
    :param pd.Dataframe master_table: Existing feature set at a timestamp-airport level
    :param pd.Dataframe df: Dataframe containing the runway departure data
    ...
    for airport in master_table['airport'].unique():
        ...
        current.rename(columns={'feat_3_flight_ind1_last': 'feat_3_cat_flight_ind1_last',
...
def ExtractLampFeatures(master_table, df):
    """
    Extracts features of weather forecasts for each airport and appends it to the 
    existing master table
    
    :param pd.Dataframe master_table: Existing feature set at a timestamp-airport level
    :param pd.Dataframe df: Dataframe containing the weather raw data
    ...
    for airport in master_table['airport'].unique():
        ...
        past_temperatures = current.groupby('timestamp').first().drop(columns = ['forecast_timestamp', 'time_ahead_prediction'])
        ...
        for p in range(1,24):
            next_temp = current[(current.time_ahead_prediction <= p) & 
...
def ExtractETDFeatures(master_table, df):
    """
    Extracts estimated time of departure features and appends it to the existing dataframe
    
    :param pd.DataFrame master_table: Existing feature set at a timestamp-airport level
    :param pd.DataFrame df: DataFrame containing the raw etd data
    ...
    for airport in master_table['airport'].unique():
        etd = df[df['airport'] == airport].copy()
        etd['timestamp'] = etd.timestamp.dt.ceil('15min')
        etd['estimated_runway_departure_time'] = pd.to_datetime(etd['estimated_runway_departure_time'])
        ...
        for i in range(1, 4 * 25):
            ...
            current = current[current['timestamp'] < current['estimated_runway_departure_time']]
            complete_etd = pd.concat([complete_etd, current])

        complete_etd['time_ahead'] = (
                complete_etd['estimated_runway_departure_time'] - complete_etd['timestamp']).dt.total_seconds()
        ...
        etd_aggregation = complete_etd.groupby('timestamp').agg({'gufi': 'count',
...
def ExtractERAFeatures(master_table, df):
    """
    Extracts estimated runway arrival features and appends them to existing master table
    
    :param pd.DataFrame master_table: Existing feature set at a timestamp-airport level
    :param pd.DataFrame df: DataFrame containing the raw era data
    
    :return pd.Dataframe master_table: Master table enlarged with additional features
    """
    
    df['estimated_runway_arrival_time'] = pd.to_datetime(df['estimated_runway_arrival_time'])

    era_features = pd.DataFrame()
    for airport in master_table['airport'].unique():
        era = df[df['airport'] == airport].copy()
        era = era[era['timestamp'] < era['estimated_runway_arrival_time']]

        complete_era = era.copy()
        for i in range(1, 4 * 25):
            current = era.copy()
            current['timestamp'] = current['timestamp'] + pd.Timedelta(f'{i * 15}min')
            current = current[current['timestamp'] < current['estimated_runway_arrival_time']]
            complete_era = pd.concat([complete_era, current])

        complete_era['time_ahead'] = (
                complete_era['estimated_runway_arrival_time'] - complete_era['timestamp']).dt.total_seconds()
        ...
        era_aggregation = complete_era.groupby('timestamp').agg({'gufi': 'count',
                                                                 'next_30min': 'sum',
                                                                 'next_60min': 'sum',
                                                                 'next_90min': 'sum',
                                                                 'next_120min': 'sum',
                                                                 ...
                                                                 'next_300min': 'sum',
                                                                 'next_330min': 'sum',
                                                                 'next_360min': 'sum'}).reset_index()

        era_aggregation.columns = ['feat_6_' + c if c != 'timestamp' else c for c in era_aggregation.columns]
...
def ExtractGufiTimestampFeatures(master_table, df, group):
    """
    ...
    :param pd.DataFrame master_table: Existing feature set at a timestamp-airport level
    :param pd.DataFrame df: DataFrame containing the raw gufi and timestamp data
    ...
    for airport in master_table['airport'].unique():
        ...
        for intervals in ['15min', '30min', '45min', '60min', '90min', '120min', 
                          '240min', '360min', '480min', '720min', '24h']:
...
def ExtractCrossSystemFeatures(df):
    """
    Extracts features of the system at the given timestep not grouping by the particular airport
    being considered in each instance, i.e. cross airport relations
    
    :param pd.Dataframe df: Dataframe containing the master table 
    
    :return pd.Dataframe df: Master table enlarged with system featuress
    """
    
    for col in df.columns:
        if 'feat' in col and '_cat_' not in col:
            max_mapping = df.groupby('timestamp')[col].max()
            mean_mapping = df.groupby('timestamp')[col].mean()
            std_mapping = df.groupby('timestamp')[col].std()
            
            df[f'feat_99_max_{col}'] = df['timestamp'].map(max_mapping)   
            df[f'feat_99_mean_{col}'] = df['timestamp'].map(mean_mapping)  
            df[f'feat_99_std_{col}'] = df['timestamp'].map(std_mapping)  
